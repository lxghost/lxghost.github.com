...
**new in 0\.5** The badge sits just under the
...
1. Clone the documentation site and change `baseURL` in `hugo.yml`
1. `hugo server` to preview **port 1313**
{.steps}
...
- **Hugo Module** (recommended) — One `hugo mod get` and you are done **needs Go**
- **Offline archive** — Builds on a machine with no network **manual upgrades**
...
Current version [**v0\.5**](/blog/);
for the upgrade steps see [**Upgrading**](/docs/admin/upgrade/).
