...
\/ jumps to search;
t toggles light and dark;
l cycles through languages.
...
⌘ + Shift + P and Ctrl + Shift + P are the same action.
For a literal plus, treat it as a key of its own: Ctrl + \+ zooms the page in.
...
On macOS press ⌘ + K; on Windows and Linux, Ctrl + K.
...
1. Type `>` for the command-only state, or type a keyword to search
1. Select with ↑ ↓ and press Enter to go
1. Escape closes it and focus returns where it was
{.steps}
...
- One sequence is one set of keys pressed together: press-A-then-B is two `kbd`
  calls and a sentence — press Escape, then Enter.
...
- [Command palette](/docs/customize/panel/) — what Ctrl + K opens
