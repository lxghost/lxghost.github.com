...
> 用 `kbd` 写快捷键：一个 shortcode 接一串按键名，输出语义化的按键序列，打印与 Markdown 输出里同样可读。
...
⌘ + Shift + P 与 Ctrl + Shift + P 是同一个动作。
需要按字面的加号时，把它当成独立的一个按键：Ctrl + \+ 放大页面。
...
macOS 按 ⌘ + K，Windows 与 Linux 按 Ctrl + K。
...
- [命令面板](/zh/docs/customize/panel/) — Ctrl + K 打开的面板
